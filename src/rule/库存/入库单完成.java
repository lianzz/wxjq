...
public class 入库单完成 extends RuleEngine{
	private static Logger logger = Logger.getLogger(入库单完成.class);
	AProxy a = new AProxy();
	...
	private Object getReturnValue(IDataContext context, IDataSet dataset,
		...
		try {
			...
			// ###################################################################################################
			/**
			 * A-入库单-完成-10 (1763254) 2013-04-10
			 */
			...
			Atzchurukjhd rkjhd = (Atzchurukjhd) dataset.getObject(Atzchurukjhd.class,
					rkjhdid); // 入库计划
			if(rkjhd == null) {
			...
			if(!"25".equals(churukyy) && !"4".equals(churukyy)
				...
				for(int i = 0; i < rkdmxs.size(); i++) {
					...
					sngl = wljcxx.getSngl();
					if(sngl == null) {
						message = "第 " + (i + 1) + " 条入库单明细物料 (" + wuliaobm
								+ ") 是否SN管理信息未设置, 无法入库！";
						returnMsg.set("NO", message);
						return returnMsg;
					}
					...
					if("1".equals(sngl)) {
						...
						cfsnmxs = dataset.getList("Atzchurukdmx", "atzchurukdid=" + rkdid
								+ " and sn='" + sn + "'");
						if(cfsnmxs != null && cfsnmxs.size() > 1) {
							message = "第 " + (i + 1) + " 条出库单明细物料 (" + wuliaobm
									+ ") 存在重复输入的 SN (" + sn + "), 请检查！";
							returnMsg.set("NO", message);
							return returnMsg;
						}
						...
						sbxx = (Atzshebeixx) dataset.getObjectByHql("Atzshebeixx",
								"from Atzshebeixx where wuliaoid=" + wuliaoid + " and sn='"
										+ sn + "'");
			...
			if(!"4".equals(churukyy) && !"8".equals(churukyy)) {
				...
				if(jianyandid != null) {
					...
					for(int i = 0; i < rkdmxs.size(); i++) {
						...
						jianyandmx = (Atzjianyandmx) dataset.getObject(Atzjianyandmx.class,
								jianyandmxid);
			...
			if(crkyy == 56 || crkyy == 34 || crkyy == 36 || crkyy == 38
					|| crkyy == 40 || crkyy == 42 || crkyy == 54 || crkyy == 16
					|| crkyy == 52) {
				Atzchurukjhd glckjhd = (Atzchurukjhd) dataset.getObjectByHql(
						"Atzchurukjhd",
			...
			if(weibaosqdid != null) {
				Atzweibaosbsqd wbd = (Atzweibaosbsqd) dataset.getObject(
						Atzweibaosbsqd.class, weibaosqdid);
				
				String weibaodlx = wbd.getWeibaodlx(); // 维保単类型: 5-维保设备入库申请单
				// -----维修-----
				if("5".equals(weibaodlx)
						|| ("2".equals(weibaodlx) && "RE".indexOf(wbd.getBianhao()) > 0)) {
					...
					for(int i = 0; i < rkdmxs.size(); i++) {
						...
						if(sn != null) {
							shebeisqdmx = (Atzshebeisqdmx) dataset.getObjectByHql(
									"Atzshebeisqdmx", "from Atzshebeisqdmx where weibaosbsqdi="
											+ weibaosqdid + " and snhao='" + sn + "'");
							
							if(shebeisqdmx == null) {
								message = "第 " + (i + 1)
										+ " 条入库单明细输入的 SN 与维保申请单中的 SN 不一致, 请检查！";
			...
			if("25".equals(churukyy)) {
				for(int i = 0; i < rkdmxs.size(); i++) {
					...
					hql = "from Atzkucun where xiangmuid=" + xiangmuid + " and wuliaoid="
							+ wuliaoid + " and kuweiid=" + kuweiid;
					...
					ptkucun.setKucunzl((new BigDecimal(Double.toString(ptl))
			...
			if(!"25".equals(churukyy)) {
				for(int i = 0; i < rkdmxs.size(); i++) {
					...
					if(ptkucun != null) {
						ptkucun.setKeyongl((new BigDecimal(Double.toString(ptkucun
								.getKeyongl())).add(new BigDecimal(Double.toString(rksl))))
								.doubleValue());
						ptkucun.setKucunzl((new BigDecimal(Double.toString(ptkucun
								.getKucunzl())).add(new BigDecimal(Double.toString(rksl))))
								.doubleValue());
					...
					if(jianyandmxid != null) {
						...
						jianyandmx.setYrkshuliang((new BigDecimal(Double
								.toString(jianyandmx.getYrkshuliang())).add(new BigDecimal(
								Double.toString(rksl)))).doubleValue());
						jianyandmx
								.setWeirksl((new BigDecimal(Double.toString(jianyandmx
										.getWeirksl())).subtract(new BigDecimal(Double
										.toString(rksl)))).doubleValue());
			...
			for(int i = 0; i < rkdmxs.size(); i++) {
				...
				rkjhdmx = (Atzchurukjhdmx) dataset.getObject(Atzchurukjhdmx.class,
						rkjhmxid);
				...
				rkjhdmx.setWeicrksl((new BigDecimal(Double.toString(weicrksl))
						.subtract(new BigDecimal(Double.toString(rksl)))).doubleValue());
			...
			if("4".equals(churukyy)) {
				...
				for(int i = 0; i < rkdmxs.size(); i++) {
					...
					ztkucun.setKeyongl((new BigDecimal(Double.toString(ztkucun
							.getKeyongl())).subtract(new BigDecimal(Double.toString(rksl))))
							.doubleValue());
					ztkucun.setKucunzl((new BigDecimal(Double.toString(ztkucun
							.getKucunzl())).subtract(new BigDecimal(Double.toString(rksl))))
							.doubleValue());
			...
			if("2".equals(churukyy)) {
				...
				for(int i = 0; i < rkdmxs.size(); i++) {
					...
					if(sczxkucun != null) {
						sczxkucun
								.setKeyongl((new BigDecimal(Double.toString(sczxkucun
										.getKeyongl())).subtract(new BigDecimal(Double
										.toString(rksl)))).doubleValue());
						sczxkucun
								.setKucunzl((new BigDecimal(Double.toString(sczxkucun
										.getKucunzl())).subtract(new BigDecimal(Double
										.toString(rksl)))).doubleValue());
			...
			if("12".equals(churukyy)) {
				...
				for(int i = 0; i < rkdmxs.size(); i++) {
					...
					if(cszxkucun == null) {
						message = "第 " + (i + 1) + " 行入库单明细物料 (" + wuliaobm
								+ ") 没有对应的测试在线库存信息！";
					...
					cszxkucun.setKeyongl((new BigDecimal(Double.toString(cszxkucun
							.getKeyongl())).subtract(new BigDecimal(Double.toString(rksl))))
							.doubleValue());
					cszxkucun.setKucunzl((new BigDecimal(Double.toString(cszxkucun
							.getKucunzl())).subtract(new BigDecimal(Double.toString(rksl))))
							.doubleValue());
				...
				for(int i = 0; i < rkdmxs.size(); i++) {
					...
					if(sn != null) {
						...
						if("9".equals(shifouzk)) {
							message = "第 " + (i + 1) + " 条入库单明细物料 (" + wuliaobm + ") 的 SN ("
									+ sn + ") 已报废 (请查看设备信息管理)！";
			...
			// -----维护入库计划是否完成字段-----
			Double wrksl = Double.parseDouble(dataset.sum("Atzchurukjhdmx",
					"weicrksl", "atzchurukjhdid=" + rkjhdid));
			...
			if(jianyandid != null) {
				jianyand = (Atzjianyand) dataset.getObject(Atzjianyand.class,
						jianyandid);
			}
			...
			if("4".equals(churukyy)) {
				...
				Atzcaigouht cght = (Atzcaigouht) dataset.getObject(Atzcaigouht.class,
						caigouhtid); // 采购合同
				...
				for(int i = 0; i < rkdmxs.size(); i++) {
					...
					cghtmx = (Atzcaigouhtmx) dataset
							.getObjectByHql(
									"Atzcaigouhtmx",
									"from Atzcaigouhtmx where id in (select caigouhtmxid from Atzjianyandmx where id="
											+ rkdmx.getJianyandmxid() + ")");
					
					cghtmx.setWeirksl((new BigDecimal(
							Double.toString(cghtmx.getWeirksl())).subtract(new BigDecimal(
							Double.toString(rksl)))).doubleValue());
					...
					// " and wuliaoid=" + wuliaoid);
					cggz = (Atzcaigougz) dataset
							.getObjectByHql(
									"Atzcaigougz",
									"from Atzcaigougz where caigouhtid="
											+ caigouhtid
					...
					cggz.setShijidhsl((new BigDecimal(
							Double.toString(cggz.getShijidhsl())).add(new BigDecimal(Double
							.toString(rksl)))).doubleValue()); // 实际到货数量
					...
					String cgxqsql = "from Atzcaigoujh where danjulx='0' and "
							...
							+ "  id in(select caigoujhmxid from Atzcaigouhtmx where id="
							+ cghtmx.getId() + "))) and id in (select caigoujh from Atzcaigoujhmx" +
									" where wuliaoid="+rkdmx.getWuliaoid()+")";
					
					List<Atzcaigoujh> cgxqlist = dataset.getListByHql("Atzcaigoujh",
							cgxqsql);
					if(cgxqlist != null && cgxqlist.size() > 0) {
						for(int j = 0; j < cgxqlist.size(); j++) {
							...
							if(tdyg != null) {
								...
								String zhuti = "到货通知:编号为" + cgxqbh + "的采购需求,编码为"
										+ rkdmx.getWuliaobm() + "的物料已到货,到货数量为：" + rksl;
								//rkdmx.getWuliaoid()
								...
								logger
										.debug("提醒流程提交：" + re.getKey() + "；提示信息：" + re.getValue());
							}
						}
					}
				...
				Double htsum = Double.parseDouble(dataset.sum("Atzcaigouhtmx",
						"weirksl", "caigouhtid=" + caigouhtid));
				if(htsum.doubleValue() == 0.0) {
					// -生成待付款信息-
					List<Atzhetongfkjd> htfkjds = dataset.getListByHql("Atzhetongfkjd",
							"from Atzhetongfkjd where atzcaigouhtid=" + caigouhtid
									+ " and fukuanlx='2'"); // 合同付款阶段List
					...
					if(htfkjds != null && htfkjds.size() > 0) {
						...
						for(int i = 0; i < htfkjds.size(); i++) {
							...
							dfkxx.setFukuanje(Double.parseDouble(df.format(
									cght.getHetongzj() * htfkjd.getFukuanbl()).toString())); // 付款金额
				...
				Atzcaigoujh cgjh = (Atzcaigoujh) dataset.getObject(Atzcaigoujh.class,
						cgjhid); // 采购计划
				...
				List<Atzcaigoujhmx> caigoujhmxs = dataset.getList("Atzcaigoujhmx",
						"caigoujh=" + cgjhid); // 采购计划明细List
				...
				if(flag) {
					...
					List<Atzchurukjhd> jhds = dataset.getList("Atzchurukjhd",
							"caigoujhid=" + cgjhid);
			...
			for(int i = 0; i < rkdmxs.size(); i++) {
				 ...
				 * "采购入库时, 到期日期必须维护！"; returnMsg.set("NO", message); return returnMsg; }
				 * }
				 */
			...
			String rValue = execAdjustmentRule("A-生成正式编码规则-05", null, context,
			...
			if(jianyandid != null) {
				Double zrksl = Double.parseDouble(dataset.sum("Atzchurukdmx",
						"shuliang",
						"atzchurukdid in (select id from Atzchurukd where jianyandid="
								+ jianyandid + " and shifouzx='1')")); // 总入库数量
				Double zshidiansl = Double
						.parseDouble(dataset.sum("Atzjianyandmx", "shidiansl",
								"atzjianyandid=" + jianyandid + " and jianyanjg!='2'")); // 总实点数量
			...
			// -----借用领用信息维护-----
			if("38".equals(churukyy) || "40".equals(churukyy)
					|| "42".equals(churukyy) || "34".equals(churukyy)
					|| "36".equals(churukyy) || "16".equals(churukyy)
					|| "46".equals(churukyy) || "48".equals(churukyy)
				...
				for(int i = 0; i < rkdmxs.size(); i++) {
					...
					if("1".equals(sngl)) {
						...
						jylyqingdan = (Atzjylyqingdan) dataset.getObjectByHql(
								"Atzjylyqingdan", jylyhql);
						
						if(jylyqingdan != null) {
							ckr = jylyqingdan.getChurukr();
							
							if(!rkr.equals(ckr)) {
								message = "第 " + (i + 1) + " 条入库单明细的 SN 借出领用人是 (" + ckr
										+ "), 请查看借用领用信息！";
						...
						if("38".equals(churukyy)) {
							...
							jylyhql += " and (yuanyin='15' or yuanyin='17') and churukr='"
									+ rkr + "'";
						} else {
			...
			if(scjhid != null) {
				...
				for(int i = 0; i < rkdmxs.size(); i++) {
					...
					scjhmx = (Atzscjhmx) dataset.getObjectByHql("Atzscjhmx",
							"from Atzscjhmx where atzscjhid=" + scjhid + " and wuliaoid="
									+ wuliaoid + " and yfszid is not null");
				...
				if(wljcxx != null && "1".equals(wljcxx.getSngl())) {
					if(sn != null && !"".equals(sn)) {
						...
						if("25".equals(churukyy)) {
							...
							mxsql = "from Atzkucunmx where kucunid=" + zxkucun.getId()
									+ " and wuliaoid=" + wuliaoid + " and sn='" + sn.trim() + "'";
							Atzkucunmx nkcmx = (Atzkucunmx) dataset.getObjectByHql(
									"Atzkucunmx", mxsql);
							if(nkcmx != null)
			...
			if("6".equals(churukyy)) {
				Atzchurukjhd rkjhd06 = (Atzchurukjhd) dataset.getObject(
						Atzchurukjhd.class, instance.getChurukjhdid());
				boolean flag06 = false;
				if(rkjhd06 != null && "1".equals(rkjhd06.getShifouwc())) {
					Atzweibaosbsqd rkwbd06 = (Atzweibaosbsqd) dataset.getObject(
							Atzweibaosbsqd.class, rkjhd06.getWeibaosqdid());
					if(rkwbd06 != null) {
						
						List<Atzchurukjhd> ckjhdList06 = dataset
								.getListByHql(
										"Atzchurukjhd",
										" from Atzchurukjhd where weibaosqdid  in (select id from Atzweibaosbsqd where bianhao='"
												+ rkwbd06.getBianhao().substring(0,
														rkwbd06.getBianhao().length() - 1) + "')");
						...
						if(flag06) {
							...
							List<Atzchurukls> cklsList06 = dataset.getListByHql(
									"Atzchurukls",
									" from Atzchurukls where churukjhdid=" + jhd06.getId());
							List<Atzchurukls> rklsList06 = dataset.getListByHql(
									"Atzchurukls", " from Atzchurukls where churukjhdid="
											+ rkjhd06.getId());
								...
								if(ckls06 != null && ckls06.getSn() != null) {
									...
									if(rklsList06 != null && rklsList06.size() > i06) {
										...
										if(rkls06 != null && rkls06.getSn() != null) {
											...
											// 执行更新
											String update06 = "update Atzshebeipzxx set wuliaosn    = '"
													+ ckls06.getSn()
													+ "',wuliaoid="
													+ ckls06.getWuliaoid()
													+ " where  wuliaosn='"
			...
			if("58".equals(churukyy)) {
				...
				if(rValue.equals("请先设置编码规则") || rValue.equals("流水号溢出")) {
					...
					List<Atzchurukdmx> mxs = dataset.getList("Atzchurukdmx",
							"atzchurukdid=" + instance.getId());
