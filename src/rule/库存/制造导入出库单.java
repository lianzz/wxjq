...
public class 制造导入出库单 extends RuleEngine {
	private static Logger logger = Logger.getLogger(制造导入出库单.class);
	AProxy a = new AProxy();
				...
				try {
					...
					if (rows > 1) {
						for (int i = 1; i < rows; i++) {
							String fahuotzdbhStr = sheet.getCell(0, i) == null ? ""
									: sheet.getCell(0, i).getContents().trim(); // 发货通知单编号
							String hetongbhStr = sheet.getCell(1, i) == null ? ""
									: sheet.getCell(1, i).getContents().trim(); // 合同编号
							String hetongmcStr = sheet.getCell(2, i) == null ? ""
									: sheet.getCell(2, i).getContents().trim(); // 合同名称
							String wuliaoStr = sheet.getCell(3, i) == null ? ""
									: sheet.getCell(3, i).getContents().trim(); // 物料号
							String wuliaomsStr = sheet.getCell(4, i) == null ? ""
									: sheet.getCell(4, i).getContents().trim(); // 物料描述
							String danweiStr = sheet.getCell(5, i) == null ? ""
									: sheet.getCell(5, i).getContents().trim(); // 单位
							String fahuoslStr = sheet.getCell(6, i) == null ? ""
									: sheet.getCell(6, i).getContents().trim(); // 发货数量
							String snStr = sheet.getCell(7, i) == null ? "" : sheet.getCell(7, i).getContents().trim(); // sn
							String fahuosjStr = sheet.getCell(8, i) == null ? ""
									: sheet.getCell(8, i).getContents().trim(); // 发货时间
							String beizhuStr = sheet.getCell(9, i) == null ? ""
									: sheet.getCell(9, i).getContents().trim(); // 备注
							Date d = null;
							...
							if (!"".equals(fahuotzdbhStr)) {
								...
								List<Atzchurukjhd> jhdList1 = dataset.getListByHql("Atzchurukjhd",
								...
								if (jhdList1 == null || jhdList1.size() <= 0) {
									ccflag = true;
									ccts.append("第").append(i + 1).append("行, 根据发货通知单或维保申请单查找出库计划失败，请检查数据;<br>");
								} else {
									Atzchurukd churukd = null;
									// 产生出库单
									List<Atzchurukd> churukdList = dataset.getListByHql("Atzchurukd",
											" from Atzchurukd where zhixingf='1' and danjulx='1' and churukjhdid in (select id from Atzchurukjhd where zhixingf='1' and (fahuotzdid in (select id from Atzfahuotzd where bianhao='"
													+ fahuotzdbhStr.trim()
													+ "') or weibaosqdid  in (select id from Atzweibaosbsqd where bianhao='"
													+ fahuotzdbhStr.trim() + "')) )");
									if (churukdList != null && churukdList.size() > 0) {
										...
										String ckdbianhao = execAdjustmentRule("A-生成正式编码规则-05", null, context, dataset,
												request, paramMap);

										List<Atzchurukjhd> jhdList = dataset.getListByHql("Atzchurukjhd",
												" from Atzchurukjhd where zhixingf='1' and danjulx='1' and (fahuotzdid in (select id from Atzfahuotzd where bianhao='"
														+ fahuotzdbhStr.trim()
														+ "') or weibaosqdid in(select id from Atzweibaosbsqd where bianhao='"
														+ fahuotzdbhStr.trim() + "'))");
										...
										churukd.setDanjulx("1"); // 出入库单类型: 出库
										...
										fahuozj.setTongzhidxfrq(new java.util.Date()); // 通知单下发日期
									...
									hqlqd = hqlqd + " danjulx ='1' and zhixingf='1' ";
									double fahuosl = Double.parseDouble(fahuoslStr);
									Atzwuliaojcxx wuliao = (Atzwuliaojcxx) dataset.getObjectByHql("Atzwuliaojcxx",
											" from Atzwuliaojcxx where  wuliaosjxz='0' and wuliaobm='"
													+ wuliaoStr.trim() + "'");
									if (wuliao == null) {
										ccflag = true;
										ccts.append("第").append(i + 1).append("行, 物料号找不到对应的物料数据;<br>");
										continue;
									}
									// 发货通知单,维保单优先查看是否有相同物料
									logger.debug(
											hqlqd + " and   (fahuotzdid in (select id from Atzfahuotzd where bianhao='"
													+ fahuotzdbhStr.trim()
													+ "') or weibaosqdid  in (select id from Atzweibaosbsqd where bianhao='"
													+ fahuotzdbhStr.trim() + "')))"
									...
									List<Atzchurukjhdmx> htfhwuliaoyyList = dataset.getListByHql("Atzchurukjhdmx",
											hqlqd + " and   (fahuotzdid in (select id from Atzfahuotzd where bianhao='"
													+ fahuotzdbhStr.trim()
													+ "') or weibaosqdid  in (select id from Atzweibaosbsqd where bianhao='"
													+ fahuotzdbhStr.trim() + "')))"
									...
									if (htfhwuliaoyyList != null && htfhwuliaoyyList.size() > 0) {
										for (Atzchurukjhdmx mx : htfhwuliaoyyList) {
											...
											if (Double.compare(fahuosl, zhizaowzxl) > 0 && zhizaowzxl != 0d) {
												...
												ckdmx.setWeizxsl(fahuosl);// 未装箱数量
												rkls.setKuweiid(mx.getKuweiid());// 库位ID
												rkls.setBencijhsl(fahuosl); // 计划数量
												rkls.setShuliang(fahuosl); // 数量

												mx.setZhizaowzxl(new BigDecimal(zhizaowzxl)
									...
									if (Double.compare(fahuosl, 0d) > 0) {
										List<Atzchurukjhdmx> htfhwuliaoxsbmList = dataset.getListByHql("Atzchurukjhdmx",
												hqlqd + " and   (fahuotzdid in (select id from Atzfahuotzd where bianhao='"
														+ fahuotzdbhStr.trim()
														+ "') or weibaosqdid in (select id from Atzweibaosbsqd where bianhao='"
														+ fahuotzdbhStr.trim() + "') ))"
										...
										if (htfhwuliaoxsbmList != null && htfhwuliaoxsbmList.size() > 0) {
											for (Atzchurukjhdmx mx : htfhwuliaoxsbmList) {
												...
												if (Double.compare(fahuosl, zhizaowzxl) > 0 && zhizaowzxl != 0d) {
													...
													ckdmx.setBencijhsl(mx.getZhizaowzxl()); // 计划数量
													ckdmx.setShuliang(mx.getZhizaowzxl()); // 实发数量
													ckdmx.setWeizxsl(mx.getZhizaowzxl());
													rkls.setKuweiid(mx.getKuweiid());// 库位ID
													rkls.setBencijhsl(mx.getZhizaowzxl()); // 计划数量
													rkls.setShuliang(mx.getZhizaowzxl()); // 数量
													mx.setZhizaowzxl(0d);
													fahuosl = new BigDecimal(fahuosl)
															.subtract(new BigDecimal(zhizaowzxl)).doubleValue();
									...
									if (Double.compare(fahuosl, 0d) > 0) {
										...
										if ("A".equals(wuliao.getAbcfl())) {
											...
											List<Atzchurukjhdmx> syhtfhwuliaoyyList = dataset.getListByHql(
													"Atzchurukjhdmx",
													hqlqd + " and   (fahuotzdid in (select id from Atzfahuotzd where bianhao='"
															+ fahuotzdbhStr.trim()
															+ "') or weibaosqdid  in (select id from Atzweibaosbsqd where bianhao='"
															...
															+ " and wuliaoid in ( select id from Atzwuliaojcxx where wuliaobm='"
															+ wuliaoStr.trim() + "')");
											if (syhtfhwuliaoyyList != null && syhtfhwuliaoyyList.size() > 0) {
												Atzchurukjhdmx mx = syhtfhwuliaoyyList.get(0);
												Atzchurukdmx ckdmx = new Atzchurukdmx();// 产生的出库明细
												ckdmx.setWuliaoid(wuliao.getId()); // 物料ID
												ckdmx.setLururq(new Date()); // 录入日期
												ckdmx.setSn(snStr);
												ckdmx.setMeidwsl(null);
												ckdmx.setAtzchurukdid(churukd.getId());
												ckdmx.setKuweiid(mx.getKuweiid()); // 库位ID
											...
											if (Double.compare(fahuosl, 0d) > 0) {
												List<Atzchurukjhdmx> syhtfhwuliaoxsbmList = dataset.getListByHql(
														"Atzchurukjhdmx",
														hqlqd + " and   (fahuotzdid in (select id from Atzfahuotzd where bianhao='"
																+ fahuotzdbhStr.trim()
																+ "') or weibaosqdid in (select id from Atzweibaosbsqd where bianhao='"
																...
																+ " and wuliaoid in ( select id from Atzwuliaojcxx where xiaoshoubmid ="
																+ wuliao.getXiaoshoubmid() + ")");
												if (syhtfhwuliaoxsbmList != null && syhtfhwuliaoxsbmList.size() > 0) {
									...
									if (Double.compare(fahuosl, 0d) > 0) {
										ccflag = true;
										ccts.append("第").append(i + 1).append("行, 找不到对应的物料编码或者相同销售编码的物料编码;<br>");

									}
									// 若为sn不为空，在设备信息中增加一条数据，状态为:已发货
									if (snStr != null && !"".equals(snStr)) {
										Atzshebeixx sbxx = (Atzshebeixx) dataset.getObjectByHql("Atzshebeixx",
												"from Atzshebeixx where sn ='" + snStr + "'");
										if (sbxx != null) {
											sbxx.setShifouzk("2");
											dataset.update(sbxx);
										} else {
											...
											sbxx.setSn(snStr);
											sbxx.setLurur((String) request.getSession().getAttribute("employeeName"));
