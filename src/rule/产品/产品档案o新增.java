package rule.产品;

import java.util.*;
import java.sql.*;
import org.apache.log4j.Logger;
import com.actiz.bcs.rule.application.engine.*;
import com.actiz.bcs.rule.application.newengine.util.AProxy;
import com.actiz.platform.application.dataset.pojo.*;

import javax.servlet.http.HttpServletRequest;
import com.actiz.platform.application.dataset.IDataSet;
import com.actiz.platform.common.datacontext.IDataContext;
import com.actiz.resource.workflow.application.WorkflowAppHelper;

@SuppressWarnings("unused")
public class 产品档案o新增 extends RuleEngine{
	private static Logger logger = Logger.getLogger(产品档案o新增.class);
	AProxy a = new AProxy();
	AProxy A = a;

	private Object getReturnValue(IDataContext context, IDataSet dataset,
			HttpServletRequest request, Map paramMap, Logger logger,
			// 实体名字
			Atzchanpinda instance) throws Exception{
		try {
			// 自定义规则
//###################################################################################################
			/**
			 * AN-产品档案-新增-12
			 * 新增产品档案发送公告
			 */
			a.setCreateInfo(instance,request);
			Atzgonggao gonggao = new Atzgonggao();
			gonggao.setZhuti("产品档案新增");
			gonggao.setNeirong("新增一条名称为"+instance.getDanganmc()+"的产品档案");
			a.setCreateInfo(gonggao, request);
			dataset.add(gonggao);
			returnMsg.set("OK","新增成功");
			return returnMsg;
//###################################################################################################
			// over
		} catch (Exception e) {
			// TODO: handle exception
		}
		return "error";
	}
}
