...
public class 附配件导入 extends RuleEngine{
	private static Logger logger = Logger.getLogger(附配件导入.class);
	AProxy a = new AProxy();
	AProxy A = a;
...
//###################################################################################################
		// 得到上传文件
					...
					try {
						...
						//表头信息
						String chanpinxh = sheet.getCell(1, 0) == null ? "" : sheet.getCell(1, 0).getContents().trim();
						String peizhimc = sheet.getCell(3,0) == null ? "" : sheet.getCell(3,0).getContents().trim();
						String peizhism = sheet.getCell(5,0) == null ? "" : sheet.getCell(5,0).getContents().trim();
						if("".equals(chanpinxh)){
							returnMsg.set("NO", "产品型号不能为空，请检查");
							return returnMsg;
						}
						Atzchanpinn chanpin = (Atzchanpinn)dataset.getObjectByHql("Atzchanpinn","from Atzchanpinn where chanpinxh='"+chanpinxh+"'");
						if(chanpin == null){
							returnMsg.set("NO", "产品型号不存在，请检查");
							return returnMsg;
						}
						if("".equals(peizhimc)){
						...
						Atzchanpinpz chanpinpz = new Atzchanpinpz();
						chanpinpz.setChanpin(chanpin.getId());
						chanpinpz.setPeizhimc(peizhimc);
						chanpinpz.setPeizhism(peizhism);
						chanpinpz.setDanjuzt("1");
						a.setCreateInfo(chanpinpz,request);
						dataset.add(chanpinpz);
