...
public class 合同信息隔绝规则 extends RuleEngine{
	private static Logger logger = Logger.getLogger(合同信息隔绝规则.class);
	AProxy a = new AProxy();
		...
		try {
			...
			if(empid != null){
				...
				if(roles != null && roles.size()>0){
					ids = "id in (-1";
					String roleName = "";
					List<Atzhetong> hetongs = new ArrayList<Atzhetong>();
					...
					for(Bc_auth_roles role : roles){
						...
						if(roleName.equals("销售人员") || roleName.equals("海外销售")){
							//客户销售经理为登陆人
							hetongs = dataset.getListByHql("Atzhetong","from Atzhetong where xiaoshoujl="+empid+" or xiaoshoujl is null");
							if(hetongs != null && hetongs.size()>0)
								for(Atzhetong hetong : hetongs)
									ids += ","+hetong.getId();
						}
						if(roleName.equals("销售助理")){
							//本部门
							hql = "from Atzhetong where xiaoshoujl in (select id from Atzemployee where bumenid="+bdep.getId()+") or xiaoshoujl is null";
							hetongs = dataset.getListByHql("Atzhetong", hql);
							if(hetongs != null && hetongs.size()>0)
								for(Atzhetong hetong : hetongs)
									ids += ","+hetong.getId(); 
						}
						if(roleName.equals("销售部总经理")){
							//本部门及下属部门
							if(fzdep != null ){
								hql = "from Atzhetong where xiaoshoujl in (select id from Atzemployee where bumenid in (select id from Atzdepartment where lujing like '"+fzdep.getLujing()+"%')) or xiaoshoujl is null";
							}else{
								hql = "from Atzhetong where xiaoshoujl in (select id from Atzemployee where bumenid in (select id from Atzdepartment where lujing like '"+bdep.getLujing()+"%')) or xiaoshoujl is null";
							}
							hetongs = dataset.getListByHql("Atzhetong", hql);
							if(hetongs != null && hetongs.size()>0)
								for(Atzhetong hetong : hetongs)
