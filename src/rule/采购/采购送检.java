...
public class 采购送检 extends RuleEngine{
	private static Logger logger = Logger.getLogger(采购送检.class);
	AProxy a = new AProxy();
	AProxy A = a;
				...
				for (int i = 0; i < daohuotzdmxList.size(); i++) {
					...
					if (rValue.equals("请先设置编码规则") || rValue.equals("流水号溢出")) {
                      ...
                      context.set("instance.atzjianyand", jianyand);
						...
						//让步接受单-12
						jianyandmx.setRangbujsdid(daohuotzdmx.getRangbujsdid());
						...
						if (daohuotzdmx.getDaohuosl() != null) {
							...
							jianyandmx.setChouyangsl(daohuotzdmx.getDaohuosl());
							jianyandmx.setHegesl(daohuotzdmx.getDaohuosl());
						} else {
							jianyandmx.setLaihuosl(daohuotzdmx.getDaohuosl());
							jianyandmx.setShidiansl(daohuotzdmx.getDaohuosl());
							jianyandmx.setChouyangsl(daohuotzdmx.getDaohuosl());
