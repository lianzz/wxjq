/**
* Copyright   :Actiz
* Version     :ODE6.0
* Net Address :www.qyes.com
* Author      :Actiz
* Build Time  :2017-06-15 13:46:46.578
*/
package com.actiz.platform.application.formdatafacility.model2;
import java.util.Date;
import com.actiz.platform.application.formdatafacility.resource.BizTypeSupport;
import com.actiz.platform.common.formdatafacility.util.SimpleObject;
import com.actiz.resource.entity.application.IAppEntityObject;
import java.sql.Timestamp;
import java.sql.Blob;
public class Ff587313300000003z implements IAppEntityObject{
	private Long atzyuezjjh_id;
	
			 		private Long atzdepartment_id;
					 		private Integer atzdepartment_cengci;
					 		private String atzdepartment_orgcode = "";
					 		private String atzdepartment_orgname = "";
					 		private String atzdepartment_yewulx = "";
					 		private Long atzdepartment_managerid;
					 		private String atzdepartment_tel = "";
					 		private Long atzdepartment_fudepartid;
					 		private String atzdepartment_lujing;
					 		private Integer atzdepartment_statuss = 0;
					 		private String atzdepartment_lurur = "";
					 		private Date atzdepartment_lururq;
					 		private String atzdepartment_zuihouxgr = "";
					 		private Date atzdepartment_zuihouxgrq;
					 		private Long atzdepartment_lururid;
					 		private Long atzdepartment_zuihouxgrid;
					 		private Long atzyuezjjh_actizversion;
					 			 		private Long atzyuezjjh_bumen;
					 		private String atzyuezjjh_zongjiemc = "";
					 		private String atzyuezjjh_jihuamc = "";
					 		private Date atzyuezjjh_zongjiekssj;
					 		private Date atzyuezjjh_zongjiejssj;
					 		private Date atzyuezjjh_jihuakssj;
					 		private Date atzyuezjjh_jihuajssj;
					 		private String atzyuezjjh_lurur = "";
					 		private Timestamp atzyuezjjh_lurusj;
					 		private String atzyuezjjh_zuihouxgr = "";
					 		private Timestamp atzyuezjjh_zuihouxgsj;
					 		private String atzyuezjjh_shenher = "";
					 		private Timestamp atzyuezjjh_shenhesj;
					 		private String atzyuezjjh_shenhezt = "";
					 		private String atzyuezjjh_shifouhz = "";
					 		private String atzyuezjjh_shyjzt = "0";
					 		public void setAtzdepartment_id(Long atzdepartment_id){
			this.atzdepartment_id = atzdepartment_id;
		}
			 	public Long getAtzdepartment_id(){
	  		return atzdepartment_id;
	  	}
	    		 		public void setAtzdepartment_cengci(Integer atzdepartment_cengci){
			this.atzdepartment_cengci = atzdepartment_cengci;
		}
			 	public Integer getAtzdepartment_cengci(){
	  		return atzdepartment_cengci;
	  	}
	    		 		public void setAtzdepartment_orgcode(String atzdepartment_orgcode){
			this.atzdepartment_orgcode = atzdepartment_orgcode;
		}
			 	public String getAtzdepartment_orgcode(){
	  		return atzdepartment_orgcode;
	  	}
	    		 		public void setAtzdepartment_orgname(String atzdepartment_orgname){
			this.atzdepartment_orgname = atzdepartment_orgname;
		}
			 	public String getAtzdepartment_orgname(){
	  		return atzdepartment_orgname;
	  	}
	    		 		public void setAtzdepartment_yewulx(String atzdepartment_yewulx){
			this.atzdepartment_yewulx = atzdepartment_yewulx;
		}
			 	public String getAtzdepartment_yewulx(){
	  		return atzdepartment_yewulx;
	  	}
	    		 		public void setAtzdepartment_managerid(Long atzdepartment_managerid){
			this.atzdepartment_managerid = atzdepartment_managerid;
		}
			 	public Long getAtzdepartment_managerid(){
	  		return atzdepartment_managerid;
	  	}
	    		 		public void setAtzdepartment_tel(String atzdepartment_tel){
			this.atzdepartment_tel = atzdepartment_tel;
		}
			 	public String getAtzdepartment_tel(){
	  		return atzdepartment_tel;
	  	}
	    		 		public void setAtzdepartment_fudepartid(Long atzdepartment_fudepartid){
			this.atzdepartment_fudepartid = atzdepartment_fudepartid;
		}
			 	public Long getAtzdepartment_fudepartid(){
	  		return atzdepartment_fudepartid;
	  	}
	    		 		public void setAtzdepartment_lujing(String atzdepartment_lujing){
			this.atzdepartment_lujing = atzdepartment_lujing;
		}
			 	public String getAtzdepartment_lujing(){
	  		return atzdepartment_lujing;
	  	}
	    		 		public void setAtzdepartment_statuss(Integer atzdepartment_statuss){
			this.atzdepartment_statuss = atzdepartment_statuss;
		}
			 	public Integer getAtzdepartment_statuss(){
	  		return atzdepartment_statuss;
	  	}
	    		 		public void setAtzdepartment_lurur(String atzdepartment_lurur){
			this.atzdepartment_lurur = atzdepartment_lurur;
		}
			 	public String getAtzdepartment_lurur(){
	  		return atzdepartment_lurur;
	  	}
	    		 		public void setAtzdepartment_lururq(Date atzdepartment_lururq){
			this.atzdepartment_lururq = atzdepartment_lururq;
		}
			 	public Date getAtzdepartment_lururq(){
	  		return atzdepartment_lururq;
	  	}
	    		 		public void setAtzdepartment_zuihouxgr(String atzdepartment_zuihouxgr){
			this.atzdepartment_zuihouxgr = atzdepartment_zuihouxgr;
		}
			 	public String getAtzdepartment_zuihouxgr(){
	  		return atzdepartment_zuihouxgr;
	  	}
	    		 		public void setAtzdepartment_zuihouxgrq(Date atzdepartment_zuihouxgrq){
			this.atzdepartment_zuihouxgrq = atzdepartment_zuihouxgrq;
		}
			 	public Date getAtzdepartment_zuihouxgrq(){
	  		return atzdepartment_zuihouxgrq;
	  	}
	    		 		public void setAtzdepartment_lururid(Long atzdepartment_lururid){
			this.atzdepartment_lururid = atzdepartment_lururid;
		}
			 	public Long getAtzdepartment_lururid(){
	  		return atzdepartment_lururid;
	  	}
	    		 		public void setAtzdepartment_zuihouxgrid(Long atzdepartment_zuihouxgrid){
			this.atzdepartment_zuihouxgrid = atzdepartment_zuihouxgrid;
		}
			 	public Long getAtzdepartment_zuihouxgrid(){
	  		return atzdepartment_zuihouxgrid;
	  	}
	    		 		public void setAtzyuezjjh_actizversion(Long atzyuezjjh_actizversion){
			this.atzyuezjjh_actizversion = atzyuezjjh_actizversion;
		}
			 	public Long getAtzyuezjjh_actizversion(){
	  		return atzyuezjjh_actizversion;
	  	}
	    		   		 		public void setAtzyuezjjh_bumen(Long atzyuezjjh_bumen){
			this.atzyuezjjh_bumen = atzyuezjjh_bumen;
		}
			 	public Long getAtzyuezjjh_bumen(){
	  		return atzyuezjjh_bumen;
	  	}
	    		 		public void setAtzyuezjjh_zongjiemc(String atzyuezjjh_zongjiemc){
			this.atzyuezjjh_zongjiemc = atzyuezjjh_zongjiemc;
		}
			 	public String getAtzyuezjjh_zongjiemc(){
	  		return atzyuezjjh_zongjiemc;
	  	}
	    		 		public void setAtzyuezjjh_jihuamc(String atzyuezjjh_jihuamc){
			this.atzyuezjjh_jihuamc = atzyuezjjh_jihuamc;
		}
			 	public String getAtzyuezjjh_jihuamc(){
	  		return atzyuezjjh_jihuamc;
	  	}
	    		 		public void setAtzyuezjjh_zongjiekssj(Date atzyuezjjh_zongjiekssj){
			this.atzyuezjjh_zongjiekssj = atzyuezjjh_zongjiekssj;
		}
			 	public Date getAtzyuezjjh_zongjiekssj(){
	  		return atzyuezjjh_zongjiekssj;
	  	}
	    		 		public void setAtzyuezjjh_zongjiejssj(Date atzyuezjjh_zongjiejssj){
			this.atzyuezjjh_zongjiejssj = atzyuezjjh_zongjiejssj;
		}
			 	public Date getAtzyuezjjh_zongjiejssj(){
	  		return atzyuezjjh_zongjiejssj;
	  	}
	    		 		public void setAtzyuezjjh_jihuakssj(Date atzyuezjjh_jihuakssj){
			this.atzyuezjjh_jihuakssj = atzyuezjjh_jihuakssj;
		}
			 	public Date getAtzyuezjjh_jihuakssj(){
	  		return atzyuezjjh_jihuakssj;
	  	}
	    		 		public void setAtzyuezjjh_jihuajssj(Date atzyuezjjh_jihuajssj){
			this.atzyuezjjh_jihuajssj = atzyuezjjh_jihuajssj;
		}
			 	public Date getAtzyuezjjh_jihuajssj(){
	  		return atzyuezjjh_jihuajssj;
	  	}
	    		 		public void setAtzyuezjjh_lurur(String atzyuezjjh_lurur){
			this.atzyuezjjh_lurur = atzyuezjjh_lurur;
		}
			 	public String getAtzyuezjjh_lurur(){
	  		return atzyuezjjh_lurur;
	  	}
	    		 		public void setAtzyuezjjh_lurusj(Timestamp atzyuezjjh_lurusj){
			this.atzyuezjjh_lurusj = atzyuezjjh_lurusj;
		}
			 	public Timestamp getAtzyuezjjh_lurusj(){
	  		return atzyuezjjh_lurusj;
	  	}
	    		 		public void setAtzyuezjjh_zuihouxgr(String atzyuezjjh_zuihouxgr){
			this.atzyuezjjh_zuihouxgr = atzyuezjjh_zuihouxgr;
		}
			 	public String getAtzyuezjjh_zuihouxgr(){
	  		return atzyuezjjh_zuihouxgr;
	  	}
	    		 		public void setAtzyuezjjh_zuihouxgsj(Timestamp atzyuezjjh_zuihouxgsj){
			this.atzyuezjjh_zuihouxgsj = atzyuezjjh_zuihouxgsj;
		}
			 	public Timestamp getAtzyuezjjh_zuihouxgsj(){
	  		return atzyuezjjh_zuihouxgsj;
	  	}
	    		 		public void setAtzyuezjjh_shenher(String atzyuezjjh_shenher){
			this.atzyuezjjh_shenher = atzyuezjjh_shenher;
		}
			 	public String getAtzyuezjjh_shenher(){
	  		return atzyuezjjh_shenher;
	  	}
	    		 		public void setAtzyuezjjh_shenhesj(Timestamp atzyuezjjh_shenhesj){
			this.atzyuezjjh_shenhesj = atzyuezjjh_shenhesj;
		}
			 	public Timestamp getAtzyuezjjh_shenhesj(){
	  		return atzyuezjjh_shenhesj;
	  	}
	    		 		public void setAtzyuezjjh_shenhezt(String atzyuezjjh_shenhezt){
			this.atzyuezjjh_shenhezt = atzyuezjjh_shenhezt;
		}
			 	public String getAtzyuezjjh_shenhezt(){
	  		return atzyuezjjh_shenhezt;
	  	}
	    		 		public void setAtzyuezjjh_shifouhz(String atzyuezjjh_shifouhz){
			this.atzyuezjjh_shifouhz = atzyuezjjh_shifouhz;
		}
			 	public String getAtzyuezjjh_shifouhz(){
	  		return atzyuezjjh_shifouhz;
	  	}
	    		 		public void setAtzyuezjjh_shyjzt(String atzyuezjjh_shyjzt){
			this.atzyuezjjh_shyjzt = atzyuezjjh_shyjzt;
		}
			 	public String getAtzyuezjjh_shyjzt(){
	  		return atzyuezjjh_shyjzt;
	  	}
	    	  	  public Long getAtzyuezjjh_id(){
  	  	return atzyuezjjh_id;
  	  }
  	  public void setAtzyuezjjh_id(Long atzyuezjjh_id){
  	  	this.atzyuezjjh_id = atzyuezjjh_id;
  	  }
	  public Long getId(){
	  	return getAtzyuezjjh_id();
	  }
	public String getObjectLabel(){
		return "";
	}
	
}